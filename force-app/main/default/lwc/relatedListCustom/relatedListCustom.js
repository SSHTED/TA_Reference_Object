...
export default class relatedListCustom extends NavigationMixin( LightningElement ) { 
    ...
    isTableVisible = true;
    isModalOpen = false;
    @api recordId; @api recordTypeId; @api relatedObjectApiName; @api relationType; @api relationPosition; @api objectApiName;
    ...
    processData(data) {
        if (data) {
            this.currentOrgType = window.location.href.includes('sandbox') ? 'dkbmc--pms.sandbox' : 'dkbmc';
    ...
    formatLevel2Data(rows, level2Columns, parentId) {
        console.log('formatLevel2Data >>> 진입');
...
*/
    ...
    get columnCount(){
        return (this.activationNo && this.activationCheckedFields) ? this.columns.length + 1
         : (this.activationNo || this.activationCheckedFields) ? this.columns.length
         : this.columns.length - 1;
    }

    get buttonLabel() {
        return this.isExpandDataTable ? '전체 접기' : '전체 펼치기';
    }
    handleExpandTableData() {
        this.isExpandDataTable = !this.isExpandDataTable;

        if (this.isExpandDataTable) {
            this.MyData.forEach((row, i) => {
                row.showDetails = true;
                row.formattedLevel2Data = this.formatLevel2Data(row.level2Data, this.level2Columns, row.Id);
            });
        } else {
            this.MyData.forEach((row, i) => {
